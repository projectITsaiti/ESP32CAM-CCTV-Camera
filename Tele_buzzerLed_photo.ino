...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i = 0; i < numNewMessages; i++) {
    ...
    if (text == "/start") {
      ...
      welcome += "\nGunakan Bot ESP32CAM Project TA Saiti ini untuk berinteraksi dengan Sistem Keamanan ini.\n";
      welcome += "\nSilahkan Klik tulisan yang bertanda '/' dibawah ini !\n";
      welcome += "/photo    : Ambil Foto\n";
      welcome += "/onflash  : LED ESP32CAM ON\n";
      welcome += "/offflash : LED ESP32CAM OFF\n";
      welcome += "/start    : start sistem\n";
    ...
    if (text == "/onflash") {
      ...
      bot.sendMessage(CHAT_ID, "LED Flash ESP32CAM dihidupkan", "");
    }
    if (text == "/offflash") {
      flashState = !flashState;
      digitalWrite(FLASH_LED_PIN, flashState);
      bot.sendMessage(CHAT_ID, "LED Flash ESP32CAM dimatikan", "");
    }
    if (text == "/photo") {
      sendPhoto = true;
